...
class ChartUtils(object):
    ...
    def createAccuracyChart(data,total_epochs):
        ...
        dataset = [[idx+1,data[idx]["accuracy"]*100,"training"] for idx in range(len(data))]
        dataset += [[idx + 1, data[idx]["val_accuracy"]*100, "test"] for idx in range(len(data))]

        min_accuracy = 0 if len(dataset)==0 else 10*(min(map(lambda x:x[1],dataset))//10)
        ...
        ax.setFontHeight(12)
        ay.setLabel("Accuracy %")
        ay.setMinValue(min_accuracy)
...
if __name__ == '__main__':
    html = ChartUtils.createAccuracyChart([{"accuracy":0.7,"val_accuracy":0.6}],10)
