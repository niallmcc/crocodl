...
from crocodl.image.classifierOLD.scorable import Scorable
...
class Trainable(object):
	...
	def train(self,foldername,training_folder,testing_folder,epoch_callback=None,epochs=5,batch_size=16,completion_callback=None,batch_callback=None):
		training_classes = list(sorted(os.listdir(training_folder)))
		if training_classes != self.classes:
			raise Exception("classes mismatch, unable to train")

		train_it = self.factory.getInputIterator(training_folder,batch_size=batch_size)
		test_it = self.factory.getInputIterator(testing_folder,batch_size=batch_size)
